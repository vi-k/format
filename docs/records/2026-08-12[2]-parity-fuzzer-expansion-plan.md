...
Статус: исполнен 2026-08-12, все шесть задач. Одно отступление от него:
правка вышла за `test/` на одну строку — `lib/src/engine.dart`
переэкспортирует `AttributeLookup` и `Representation`, без чего задача 3
не собиралась. Пороги, стоящие в тексте заглушками, заменены
измеренными; итоговые числа — в доккомментарии `_casesPerDialect`, а не
здесь, потому что здесь они разошлись бы с кодом. Дизайн, из которого
план следует, — `docs/records/2026-08-12[1]-parity-fuzzer-expansion-design.md`.
