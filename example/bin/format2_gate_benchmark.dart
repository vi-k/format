...
import 'package:example/src/utils/output.dart' show defaultFg;
...
void _run() {
  ...
  for (final scenario in gateBenchmarkScenarios) {
    final legacySamples = <double>[];
    final currentSamples = <double>[];

    for (var measurement = 0; measurement < _measurementCount; measurement++) {
      final legacy = GateLegacyFormatBenchmark(scenario)..verifyOutput();
      final current = GateFormatBenchmark(scenario)..verifyOutput();
