...
- Widened the `intl` constraint from `^0.20.2` to `>=0.19.0 <0.21.0`, so the
  package installs alongside `flutter_localizations` from Flutter 3.27.0 rather
  than 3.32.0. `flutter_localizations` pins `intl` to one exact version, and a
  caret excluded the 0.19.0 half of the range. The adapter reports whatever the
  resolved `intl` carries, and four of its 119 locales differ across the
  range — `en_ZA`, `de_CH`, `gsw` and `it_CH`, listed in the README. Three of
  those four already differed inside `^0.20.2`.
