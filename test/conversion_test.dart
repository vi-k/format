...
//
// `!s` is Dart's `toString`. `!r` and `!a` are Python's `repr` and `ascii`, and
// that is the reason most of this file exists: the spelling they produce is a
// compatibility promise, down to which quote character a string gets, which
// scalars are escaped and in what notation, and how a container is laid out.
// None of it is derivable from Dart's own conventions, so every rule is pinned
// against a literal taken from Python's behaviour rather than from ours.
//
