...
//
// The disagreement half is a registry (`python_divergences.json`) of the places
// this package intentionally differs — Dart tokens for `true`/`null`,
...
// differences rots quietly, so two tests keep it honest: every entry must point
// at a README section that exists, and every entry must have an executable
// exemplar here pinning what the package actually does. Adding a divergence to
// the documentation without demonstrating it fails the second; letting the
// behaviour drift afterwards fails the exemplar itself.
//
// The fixture decoder gets its own tests because JSON cannot express what these
// fixtures need: negative zero, NaN, infinities, `BigInt`, sets, and the
...
//
// VM-only in practice: it reads the fixture files from disk. The one divergence
// whose Dart side exists only on the web is exempted by name and pinned in
// `js_number_dispatch_test.dart` instead.
