// Compatibility with C++'s `std::sprintf`, built the same way as the Python
// one: fixtures generated by running the templates through a real
// implementation, plus a registry of the places this dialect deliberately
// differs.
//
// The divergences here are sharper than on the Python side, because C's printf
// is permissive where this one is strict — a negative value through `%u`, a
// value of the wrong type, a length modifier — and because some C behaviour is
// undefined rather than merely different. Each entry therefore records both
// outcomes, the C++ one and the Dart one, and the Dart side is split in two:
// entries that produce a different *result* and entries that produce an
// *error*. Both kinds must ship an executable exemplar, and the registry test
// checks the correspondence in both directions — an entry with no exemplar
// fails, and an exemplar with no entry fails too.
//
// The registry is validated as a document as well as a set of claims: schema
// version, ids sorted and unique, a non-empty human reason on every entry, and
// a README anchor that resolves against the actual headings. That is what keeps
// "we differ here, see the README" from becoming a broken promise.
//
...
void main() {
  ...
  // The exemplars for divergences that produce a different result rather than
  // an error. Named by registry id, so a failure names the documented claim it
  // contradicts. The error-producing exemplars run inside the registry test
  // below, where the documented exception type is compared as well.
  ...
  // The registry as a document and as a contract. Structure first — schema
  // version, every field present and of the right shape, a non-empty reason,
  // ids sorted and unique — because a registry that is hard to read is a
  // registry nobody reviews. Then the correspondence: result entries against
  // the exemplars above, error entries run here with their documented exception
  // type checked, and a required-id set so a divergence cannot be deleted from
  // the documentation while the behaviour remains.
  //
  // Finally the anchor, resolved against the README's real headings rather than
  // assumed.
