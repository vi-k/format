// Loading and decoding the compatibility fixtures — the CPython and C++ cases
// generated by `tool/` and committed under `test/fixtures/`.
//
// The fixtures are evidence about another implementation's behaviour, so the
// loader is deliberately strict rather than forgiving: a file that is
// malformed, unsorted, ambiguous about its expected outcome, or holds a value
// it cannot represent faithfully is rejected outright. A lenient loader would
// turn a generator bug into a quietly weaker test suite, which is the one
...
// travels as a `{type, value}` pair, and `decodeFixtureValue` is the only place
// that knows how to read them back. Its own tests are in
